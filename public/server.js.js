...
// ---------- Static frontend ----------
// Auto cache-busting: ASSET_VERSION is fixed when the server boots. Every
// Render deploy restarts the process, so it changes on each deploy and forces
// browsers (especially mobile) to download the newest styles.css / app.js
// instead of a stale cached copy. index.html itself is served with no-cache so
// the phone always re-reads the current version tag; the CSS/JS URLs then
// change automatically and are refetched. Between deploys the tag is stable, so
...
function sendIndex(req, res) {
  fs.readFile(path.join(__dirname, 'public', 'index.html'), 'utf8', (err, html) => {
    ...
    res.type('html').send(html.replace(/__ASSET_V__/g, ASSET_VERSION));
  });
}
app.get('/', sendIndex);
